...
That split is what lets two callers share one extraction:
    ...
    recalculate  candidacy = "did the user equip it?"

`keep_unmatched` is the recalculation path's one behavioural difference: report
every stat the gear grants, including ones no priority claims, instead of
dropping them (plan §2.4/§5.3). It defaults to False so the search's output is
...
* `<Item>` repeats — one effect can name several targets. findtext takes the
  FIRST, which is the established behaviour across 4261 effects.
...
def _raw_stat_name(b_type, b_item):
    ...
    abilitybonus" where a priority says "Intelligence". That is acceptable
    because these names only ever key `otherStats` — the stats no priority
    claims — and never `realizedStats`, which stays priority-spelled via
    normalize_stat_name (plan §3.3, decision 2).
...
def _item_provenance(item_node, name, quests_lookup, raid_names,
    ...
    if quests_lookup:
        ...
        # direct-match-only check; a direct match is still the common case
        # and resolves immediately inside _resolve_is_raid.
...
def _item_buffs_from_node(item_node, priorities, wanted_weapon_stats,
    ...
    `keep_unmatched` is the recalculation path's requirement (plan §2.4/§5.3):
    report every stat the gear provides, including ones no priority claims,
    instead of dropping them. It defaults to False so the search's output — and
    therefore the Phase 1 purity snapshot — is bit-for-bit what it always was.
    """
    buffs = []
    ...
    # filter over catalog data. Whether recalculation should report every
    # weapon base stat is a Phase 3 decision, not a Phase 1 refactor.
    try:
...
def _item_from_node(item_node, item_file, slots, priorities, wanted_weapon_stats,
    ...
    type is disallowed in Weapon2 keeps Weapon1). Recalculation passes the raw
    slot list from _item_slots_from_node.
    """
...
def _effect_buffs_from_node(effect_node, priorities, keep_unmatched=False):
    """Buffs from one `<Effect>`. Shared by augments, filigrees and filigree
    set tiers — all three read the same shape (`<Type>+ <Bonus> <Item>
    <Amount>`), and before the Phase 1 split each had its own copy of this loop.
...
def _augment_from_node(aug_node, priorities, keep_unmatched=False):
    ...
    that is a data shape check, not a search restriction.
    """
    a_type = aug_node.findtext('Type')
    if not a_type:
        return None
...
def _filigree_from_node(f_node, priorities, keep_unmatched=False):
    ...
    return {
        ...
        # Real bug, deliberately NOT fixed here: it changes solver output and
        # must land alone, after the Phase 4 baseline. Plan §2.5 / Phase 7.
