...
>
  <div class="w-full max-w-xl mx-4 rounded-lg border border-border bg-popover text-popover-foreground shadow-2xl p-5 space-y-4 max-h-[85vh] flex flex-col">
    <div class="flex items-center justify-between shrink-0">
      <span class="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Add a stat</span>
      <button
        type="button"
    ...
    <input
      ...
      class="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring shrink-0"
    />

    {#if searchTerm.length > 0}
      <div class="overflow-y-auto space-y-1 min-h-0 flex-1">
        {#each searchResults as entry (entry.stat)}
          <button
            type="button"
            class="w-full text-left rounded px-3 py-2 hover:bg-accent hover:text-accent-foreground transition-colors"
        ...
        {:else}
          <p class="text-xs text-muted-foreground px-3 py-4">
    ...
    {:else}
      <div class="flex gap-3 min-h-0 flex-1">
        ...
        <div class="w-2/5 overflow-y-auto border-r border-border pr-2 space-y-0.5">
          {#each categories as cat (cat.label)}
            <button
              type="button"
              class="w-full text-left rounded px-3 py-2 text-sm transition-colors {activeCategory === cat ? 'bg-accent text-accent-foreground font-medium' : 'hover:bg-accent/50'}"
        ...
        <div class="w-3/5 overflow-y-auto pl-1 space-y-1">
          {#if activeCategory}
            ...
            {#each subCategoriesOf(activeCategory) as sub (sub.label)}
              <div class="pt-2">
                <p class="px-3 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{sub.label}</p>
    ...
    <div class="border-t border-border pt-3 space-y-1 shrink-0">
      ...
      <div class="flex gap-2">
        <input
          ...
          class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        />
        <button
          type="button"
          class="rounded-md bg-secondary text-secondary-foreground border border-border px-4 text-xs hover:bg-secondary/80 transition-colors"
